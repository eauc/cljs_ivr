...
(defn- conform-dtmf-ok
	[case]
	(cond-> case
		(contains? case "dtmf_ok") (update "dtmf_ok" node/conform-set "set")))
...
(defmethod node/conform-type "dtmfcatch"
	...
	(-> node
			...
			conform-welcome
			(cond->
					(contains? node "case") (update "case" conform-dtmf-ok))))
...
(defn- save-action-data
	...
	(let [set (get-in node ["case" "dtmf_ok" "set"])
				new-data (-> action-data
										 (assoc varname digits)
