  ...
  (let [options {:id "node-id"
    ...
    (testing "conform"
      ...
      (is (= {"type" "dtmfcatch"
              ...
              "script_id" "script-id"
              "max_attempts" 0
              "retry" {"soundname" "try-again",
                       :type :ivr.node.dtmf-catch/sound}
              "welcome" [{:type :ivr.node.dtmf-catch/sound
                          "soundname" "son1"}]}
             (node/conform
               {"type" "dtmfcatch"
                "retry" {"soundname" "try-again"}
                "welcome" [{"soundname" "son1"}]}
               options)))
                ...
                (is (= {:ivr.routes/response
                        {:verbs :create
                         :data [{:numdigits 2
                                 ...
                                 :play [{:text "try again", :voice "robert"}
                                        {:text "hey", :voice "alice"}]}
                                {:type :ivr.verbs/redirect
                                 :path "/smartccivr/script/script-id/node/node-id/callback?retries=5"}]}}
                       (node/leave-type
                         (merge node {"max_attempts" 5
                                      "retry" {:type :ivr.node.dtmf-catch/speak
                                               "varname" "titi"
                                               "voice" "robert"
                                               "pronounce" "normal"}
                                      "welcome" [{:type :ivr.node.dtmf-catch/speak
                         ...
                         (merge context {:params {"retries" 4}
                                         :call {:action-data {"titi" "try again"
                                                              "toto" "hey"}}}))))))))))))
